  ...
  //------------------------------------------
{
  ...
  if (s==s_tracing){
    ...
    // PORT PATCH: upstream used inline "_asm int 03h", which MSVC does not
    // support on x64 (no inline asm on that target) -- __debugbreak() is
    // MSVC's portable intrinsic for the same trap. See PROVENANCE.md.
    if (s==s_debugBreak){
      __debugbreak();
      bRet=true;
    }else
