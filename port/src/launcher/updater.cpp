...
bool LooksLikeShadowkeyPackage(const std::vector<std::string>& entryPaths) {
    ...
    for (const std::string& path : entryPaths) {
        ...
        if (lowered == "shadowkey.exe") hasLauncher = true;
        if (lowered == "bin/shadowkey_port.exe") hasGame = true;
    }
...
bool InstallUpdate(const std::string& installRoot, const ReleaseInfo& release,
    ...
    // Check before unpacking: an archive that is not a Shadowkey build must
    ...
    if (!LooksLikeShadowkeyPackage(paths)) {
        error = "the downloaded archive does not look like a Shadowkey build";
        fs::remove_all(updateDir, code);
        return false;
    }
...
bool RelaunchLauncher(const std::string& installRoot) {
    const fs::path exe = fs::path(installRoot) / "Shadowkey.exe";
    std::error_code code;
