...
// M115: replacing the install with a newer one, including the executable
// doing the replacing.
//
// The awkward part is that `Shadowkey.exe` cannot overwrite itself while it
...
// RelaunchAndExit) because the running executable has just been replaced.
...
// Checks that an unpacked archive actually looks like a Shadowkey build
