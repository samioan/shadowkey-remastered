...
bool SaveLauncherConfig(const std::string& path, const LauncherConfig& config) {
    ...
    if (!file) return false;
    file << "# Shadowkey Remastered launcher settings.\n"
...
std::string ResolveAgainst(const std::string& root, const std::string& relative) {
    ...
    // Without it a stored "data/Ceurope.gdr" joins onto a Windows root as
    // `C:\...\install\data/Ceurope.gdr`, which is a perfectly valid path
    // and looks like a bug on screen next to its backslashed neighbour.
