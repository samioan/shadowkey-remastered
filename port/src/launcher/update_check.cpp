...
bool HttpGet(const std::wstring& url, const wchar_t* accept,
    ...
    // network.
    HINTERNET session = WinHttpOpen(L"ShadowkeyRemastered-Launcher/1.0",
...
bool AppendToString(const char* data, size_t size, void* context) {
    ...
    // The release payload is a few KB; a megabyte means something is very
    // wrong and we should not keep growing a string over it.
...
bool ParseReleaseList(const std::string& json, ReleaseInfo& out) {
    ...
    // The response is an array; the request asks for one element, so the
    // first occurrence of each field belongs to the same release. Bailing
    // out on an empty array keeps "[]" from being read as a malformed one.
    const size_t firstBrace = json.find('{');
    if (firstBrace == std::string::npos) return false;

    if (!ReadJsonString(json, "tag_name", firstBrace, out.tag)) return false;
    out.version = out.tag;
    if (!out.version.empty() && (out.version[0] == 'v' || out.version[0] == 'V')) {
        out.version.erase(0, 1);
    }
    ReadJsonBool(json, "prerelease", firstBrace, out.prerelease);

    // Walk the asset URLs and take the first .zip. A release can carry
    // other things -- checksums, source tarballs GitHub adds itself -- and
    // picking blind would hand the updater a .tar.gz to unzip.
    size_t search = firstBrace;
    std::string url;
    size_t urlEnd = 0;
    while (ReadJsonString(json, "browser_download_url", search, url, &urlEnd)) {
        if (EndsWith(url, ".zip")) {
            out.downloadUrl = url;
            const size_t slash = url.find_last_of('/');
            out.assetName = slash == std::string::npos ? url : url.substr(slash + 1);
            break;
        }
        search = urlEnd;
    }
...
bool FetchLatestRelease(const std::string& owner, const std::string& repo, ReleaseInfo& out,
    ...
    out = ReleaseInfo{};
    error.clear();
    // per_page=1: see this header's note. One element means the first
    // occurrence of every field in the payload belongs to that release.
    const std::wstring url = L"https://api.github.com/repos/" + Widen(owner) + L"/" +
                             Widen(repo) + L"/releases?per_page=1";
    std::string body;
    if (!HttpGet(url, L"Accept: application/vnd.github+json\r\n", AppendToString, &body, nullptr,
                 error)) {
        return false;
    }
    if (!ParseReleaseList(body, out)) {
        error = "the release list had no downloadable build in it";
        return false;
    }
    return true;
}
