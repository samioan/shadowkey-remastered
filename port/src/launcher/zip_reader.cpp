...
bool ReadWholeFile(const std::string& path, std::vector<unsigned char>& out, std::string& error) {
    ...
    // The release zip is a couple of megabytes; a cap keeps a corrupt or
    // hostile file from being read entirely into memory.
    if (size > 512ll * 1024 * 1024) {
        error = path + " is implausibly large for an update";
        return false;
    }
...
bool Inflate(const std::vector<unsigned char>& data, size_t at, const CentralEntry& entry,
    ...
    // -- which is exactly puff's input format, so unlike zone_file.cpp
