...
.DESCRIPTION
    M114. Run after port/build_dist.bat, and by the release workflow before
    anything is uploaded. Everything here is something that would otherwise
    only be discovered by a person downloading the zip:
      ...
      * no game data or font accidentally swept into the package, which
        would be a licensing problem, not just a large download
...
# --- layout -----------------------------------------------------------
$launcher = Join-Path $DistDir 'Shadowkey.exe'
$game = Join-Path $DistDir 'bin/shadowkey_port.exe'
...
# The game data and the Nokia font are the user's to supply. `data/` and
# `user/` are created at runtime and must not be in the package.
...
$strays = Get-ChildItem -Recurse -File $DistDir |
          Where-Object { $_.Extension -in @('.gdr', '.spr', '.sav', '.s', '.eng') }
foreach ($stray in $strays) { Fail "game/font data in the package: $($stray.FullName)" }
...
if (Test-Path $launcher) {
    ...
    if ($size -lt 400KB) {
        Fail ("Shadowkey.exe is only $([math]::Round($size/1KB)) KB -- too small to " +
