#!/usr/bin/env python3
"""Build the launcher's .ico from a square crop of the banner artwork.

Companion to tools/make_banner.py, and the same reasoning: the decode goes
through PowerShell's System.Drawing because that is the one image decoder
present on a Windows box without installing anything.
...
Usage:
    python tools/make_icon.py <image> [--box X,Y,W,H]
        [-o port/src/launcher/assets/shadowkey.ico]

--box selects the square region to crop before scaling, in source pixels;
the default is the dragon-key head in the committed wallpaper.
"""
...
def main():
    ...
    # The key's head in banner_source.jpg (1131x707): a square around the
    # dragon and the amethyst, which is the one part of this flat artwork
    # that still reads at 16x16.
    parser.add_argument("--box", default="466,258,200,200",
                        help="square crop as X,Y,W,H in source pixels")
    parser.add_argument("-o", "--output", default=os.path.join(
        "port", "src", "launcher", "assets", "shadowkey.ico"))
